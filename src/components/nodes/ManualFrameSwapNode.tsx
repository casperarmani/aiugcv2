...
interface ManualFrameSwapNodeProps {
  id: string;
  data: {
    onSuccess?: (output: ManualFrameSwapOutput) => void;
  };
}

export interface ManualFrameSwapOutput {
...
export function ManualFrameSwapNode({ id, data }: ManualFrameSwapNodeProps) {
  ...
  const runFaceSwap = async (e: React.FormEvent) => {
    ...
    try {
      const response = await axios.post('/api/manual-frame-swap', {
        frame0Path,
        frame1Path,
        swapImageUrl,
      });
  ...
  return (
    <div className="p-4 border rounded-lg bg-white shadow-md w-80 text-black">
      <h3 className="text-lg font-semibold mb-2">Frame Face Swap</h3>
